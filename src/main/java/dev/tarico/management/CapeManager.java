...
import dev.tarico.module.modules.fun.Loli;
import dev.tarico.module.modules.render.Capes;
...
public class CapeManager implements LayerRenderer<AbstractClientPlayer> {
    public static ResourceLocation capeLocation = null;
    private final RenderPlayer playerRenderer;

    public CapeManager(RenderPlayer playerRenderer) {
    ...
    public void doRenderLayer(AbstractClientPlayer playerEntity, float p_177141_2_, float p_177141_3_, float partialTicks, float p_177141_5_, float p_177141_6_, float p_177141_7_, float scale) {
        if (!ModuleManager.instance.getModule(Capes.class).getState() || ModuleManager.instance.getModule(Loli.class).getState())
            return;
        if (playerEntity.hasPlayerInfo() && !playerEntity.isInvisible() && capeLocation != null) {
            ...
            if (playerUuid == Minecraft.getMinecraft().thePlayer.getUniqueID()) {
                try {
                    ...
                    double d3 = MathHelper.sin(f0 * 3.1415927F / 180.0F);
                    double d4 = -MathHelper.cos(f0 * 3.1415927F / 180.0F);
