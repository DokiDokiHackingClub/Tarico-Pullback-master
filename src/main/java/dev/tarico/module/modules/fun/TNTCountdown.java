...
public class TNTCountdown extends Module {
    ...
    public TNTCountdown() {
        super("TNTCountdown", "Rendering TNT Timer Countdown", ModuleType.Fun);
    }
    ...
    public void onWorldRenderLast(RenderWorldLastEvent event) {
        ...
        for (Entity entity : entitiesById) {
            if (entity instanceof EntityTNTPrimed && entity.isInRangeToRender3d(renderingVector.getX(), renderingVector.getY(), renderingVector.getZ())) {
                renderFuseCountdown((EntityTNTPrimed) entity, partialTicks);
            }
        }
    }

    private void renderFuseCountdown(EntityTNTPrimed passedEntity, float partialTicks) {
        Minecraft mc = Minecraft.getMinecraft();
        ...
        GlStateManager.translate((float) (x - renderPosX), (float) (y - renderPosY + passedEntity.height) + (float) 0.5, (float) (z - renderPosZ));
        ...
        wr.pos(-j - 1, -1, 0.0D).color(0, 0, 0, 64).endVertex();
        wr.pos(-j - 1, 8, 0.0D).color(0, 0, 0, 64).endVertex();
        wr.pos(j + 1, (8), 0.0D).color(0, 0, 0, 64).endVertex();
        wr.pos(j + 1, (-1), 0.0D).color(0, 0, 0, 64).endVertex();
        tessellator.draw();
